...
Open **模型配置** beside the theme switch in the top bar to enter an API key and model identifier. **配置并测试连接** sends one synthetic routing question to DeepSeek (potentially billable); only a valid structured response replaces the current configuration. This verifies connection/format, not business-language accuracy. Failed tests retain the previous configuration. The key is cleared from the input on submit or close, never returned by the API, and is not written to application browser storage, business/checkpoint databases or project files. The configuration client deliberately bypasses business mutation retry receipts and their in-memory fingerprint cache.

UI configuration lives only in backend process memory. Restarting loses it and re-reads any existing server environment variables. **清除配置** disables the model for the current process only; it does not edit environment variables or erase stored forecasts/tasks. Pending clarification remains available once a model is configured again. Configuration changes are blocked while Agent tasks are queued/running or another connection test is in progress. Manual forecast calculations remain available.

`/api/v1/model-config` GET/PUT/DELETE responses use `Cache-Control: no-store`. Access requires a loopback Host, a loopback browser Origin when supplied, and `X-Haru-Config: 1`; PUT/DELETE also require JSON and the process-specific `X-Haru-CSRF` token obtained from GET. Cross-site browser requests are rejected; CORS is not enabled. These controls mitigate browser cross-site requests and DNS rebinding; they are **not user authentication**. Keep the app bound to localhost. Remote hosting requires separate HTTPS/authentication design. Browser extensions, compromised local processes, and memory inspection are outside this local demo's protections. Do not capture developer-tools network traces or screenshots containing real credentials.

Set `DEEPSEEK_API_KEY` and `DEEPSEEK_MODEL` on the API process. Use a model identifier available in your DeepSeek account and official API documentation. In Compose, copy `.env.example` to the ignored `.env`, then fill those two values; preserve any existing local settings. For local Python development, set process environment variables before starting Uvicorn (`.env` is not loaded automatically). Restart the API after changing model configuration. Never put keys in `VITE_*` variables or ordinary business requests. The model configuration dialog sends credentials only to the guarded local configuration endpoint.
...
The task list supports `offset` in pages of 20. Select an older task to reply or resume; clarification questions and replies remain visible. Legacy task dialogue is reconstructed from its saved plans and replies. Flow sources are filtered to the answer's months. Continuous periods use inclusive `month_from`/`month_to` parameters (up to 600 months) to keep request URLs short. Non-continuous selections retain repeated `months` parameters (up to 240 entries); range parameters cannot be mixed with `month` or `months`. Cash balances and funding gaps retain cumulative evidence through the relevant ending/peak month. Known foreign project names, portfolio subsets and model-reported scope mismatches trigger clarification. Unknown aliases still depend on model interpretation; always review the displayed bound project and period.
