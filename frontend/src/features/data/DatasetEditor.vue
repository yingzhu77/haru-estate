...
<template>
  ...
  <el-tabs>
    ...
    >
      ...
      <el-empty
        v-if="!modelValue[collection]?.length"
        description="尚无记录，缺项不会自动补零"
        :image-size="60"
      />
      <article
        v-for="(row, index) in modelValue[collection]"
        :key="index"
        class="record"
      >
        <div class="record-heading">
          <strong>{{ titles[collection] }} #{{ index + 1 }}</strong><el-button
            type="danger"
            link
            @click="remove(collection, index)"
          >
            移除草稿记录
          </el-button>
        </div>
        <div class="field-grid">
          ...
          >
            ...
            >
              ...
              <option
                v-for="phase in modelValue.phases"
                :key="phase.id"
                :value="phase.id"
              >
                {{ phase.name || phase.id }}
    ...
    <el-tab-pane label="经营与融资假设">
      <el-alert
        title="以下参数均为模拟假设，待业务调研确认。融资提款与还本按明确节点配置。"
        type="info"
        :closable="false"
      />
      <template v-if="modelValue.assumptions">
        <div class="field-grid assumption-grid">
          <label
            v-for="field in assumptionFields"
            :key="field.key"
        ...
        <PaymentEditor
          :model-value="modelValue.assumptions.loan_draws ?? []"
          label="融资提款计划"
          @update:model-value="changeLoans('loan_draws', $event)"
        />
        <PaymentEditor
          :model-value="modelValue.assumptions.loan_repayments ?? []"
          label="融资还本计划"
          @update:model-value="changeLoans('loan_repayments', $event)"
        />
      </template>
      <el-alert
        v-else
        title="接口未提供假设数据，请重新载入并核对，系统不会自动补零。"
        type="error"
        :closable="false"
      />
    </el-tab-pane>
