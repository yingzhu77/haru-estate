...
const rows = computed(() =>
  props.view === "lifecycle"
    ? props.result.months
    : props.result.months.filter((month) =>
        props.result.target_months.includes(month.month),
      ),
);
...
function paint() {
  ...
  const options: EChartsCoreOption = {
    ...
    series: series.map((item) => ({
      ...
      lineStyle: { width: 2.5 },
      data: rows.value.map(
        (row) => Number(row[item.field as keyof typeof row]) / 10000,
      ),
  ...
  chart.on("click", (params) => {
    ...
    if (row)
      emit(
        "inspect",
        row.month,
        props.view === "cash" ? "net_cash_flow" : "profit",
      );
  });
