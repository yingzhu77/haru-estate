...
export const scopeKey = (kind: Run["kind"], ids: string[]) =>
  `${kind}:${[...ids].sort().join(",")}`;
...
export function useWorkbench() {
  ...
  let disposed = false;
  let submission:
    { payload: string; key: ReturnType<typeof crypto.randomUUID> } | undefined;
  ...
  const currentRun = computed(() =>
    state.activeRun &&
    matchesScope(state.activeRun, state.mode, selectedIds.value)
      ? state.activeRun
      : null,
  );
  const resultRun = computed(() => {
    if (currentRun.value?.status === "completed" && currentRun.value.result)
      return currentRun.value;
    return previousRun.value &&
      matchesScope(previousRun.value, state.mode, selectedIds.value)
      ? previousRun.value
      : null;
  });
  const result = computed(() => resultRun.value?.result ?? null);
  const currentRevision = computed(
    () => revisions.value[state.selectedProjectId],
  );
  const busy = computed(() => submitting.value || isRunning(currentRun.value));
  const validation = computed(() => {
    if (!selectedIds.value.length) return "请先选择至少一个项目。";
    if (!state.forecastOrigin || !state.informationCutoff)
      return "请填写预测基准日和信息截止日。";
    if (state.informationCutoff > state.forecastOrigin)
      return "信息截止不得晚于预测基准日。";
    if (selectedIds.value.some((id) => !revisions.value[id]))
      return "所选项目的输入尚未读取完成。";
    return "";
  });
  ...
  function remember(run: Run) {
    if (run.status === "completed" && run.result) {
      ...
      if (matchesScope(run, state.mode, selectedIds.value))
  ...
  async function poll(runId: string) {
    ...
    try {
      ...
      if (
        ...
        pollError.value =
          cause instanceof Error ? cause.message : "暂时无法读取运行状态。";
      }
    }
  }
  ...
  async function loadScope() {
    ...
    if (inputs.status === "fulfilled") {
      revisions.value = Object.fromEntries(
        inputs.value.map((input) => [input.project_id, input]),
      );
      ids.forEach((id) => {
      ...
      revisions.value = {};
      error.value =
        inputs.reason instanceof Error
  ...
  watch(
    () => [state.forecastOrigin, state.informationCutoff, editor.scenario],
    markDirty,
  );
  watch(
    selectedKey,
  ...
  async function generate() {
    ...
    const request: RunCreate = {
      ...
      base_versions: Object.fromEntries(
        ids.map((id) => [id, revisions.value[id]!.version]),
      ),
    ...
    if (submission?.payload !== payload)
      submission = { payload, key: crypto.randomUUID() };
    ...
    try {
      ...
      if (!disposed && requestedScope === selectedKey.value) {
        error.value =
          cause instanceof Error ? cause.message : "提交失败，请重试。";
      }
    } finally {
  ...
  async function resume() {
    ...
    try {
      ...
      error.value =
        cause instanceof Error ? cause.message : "恢复失败，请查看历史记录。";
    } finally {
      submitting.value = false;
    }
  }
  ...
  return {
    ...
    markDirty,
    refreshStatus: () =>
      currentRun.value ? poll(currentRun.value.id) : Promise.resolve(),
  };
}
