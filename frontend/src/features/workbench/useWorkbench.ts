...
export const scenarioLabels = { base: '基准', optimistic: '乐观', prudent: '审慎' }
export const statusLabels: Record<string, string> = {
  queued: '等待计算', running: '计算中', completed: '已完成', failed: '失败',
  incomplete: '汇总未完成', interrupted: '中断待恢复',
}
export const isRunning = (run: Run | null) => !!run && ['queued', 'running'].includes(run.status)
...
export const defaultOverrides = (): Overrides => ({
  price_change: '0', remaining_cost_change: '0', collection_delay: 0, delivery_delays: {},
})
...
export function useWorkbench() {
  ...
  let disposed = false
  let submission: { payload: string; key: string } | undefined

  const selectedIds = computed(() => state.mode === 'project'
    ? (state.selectedProjectId ? [state.selectedProjectId] : [])
    : state.selectedProjectIds)
  const selectedKey = computed(() => scopeKey(state.mode, selectedIds.value))
  const currentRun = computed(() => state.activeRun &&
    matchesScope(state.activeRun, state.mode, selectedIds.value) ? state.activeRun : null)
  const resultRun = computed(() => currentRun.value?.status === 'completed' && currentRun.value.result
    ? currentRun.value : previousRun.value)
  ...
  async function poll(runId: string) {
    ...
    try {
      ...
      if (disposed || watchedScope !== selectedKey.value || state.activeRun?.id !== runId) return
      state.activeRun = fresh
      remember(fresh)
      pollError.value = ''
      if (isRunning(fresh)) pollTimer = setTimeout(() => void poll(runId), 1500)
    } catch (cause) {
      if (!disposed && state.activeRun?.id === runId) {
  ...
  async function loadScope() {
    ...
    if (inputs.status === 'fulfilled') {
      ...
      ids.forEach(id => { if (!state.overrides[id]) state.overrides[id] = defaultOverrides() })
    } else {
      revisions.value = {}
      error.value = inputs.reason instanceof Error ? inputs.reason.message : '项目输入读取失败。'
    }
    if (runs.status === 'fulfilled') {
      const matching = runs.value.filter(run => matchesScope(run, mode, ids))
  ...
  function markDirty() { dirtySinceResult.value = true }
  watch(() => [state.forecastOrigin, state.informationCutoff, editor.scenario], markDirty)
  watch(selectedKey, () => { dirtySinceResult.value = false; void loadScope() }, { immediate: true })
  ...
  async function generate() {
    ...
    const request: RunCreate = {
      kind: state.mode, project_ids: ids, forecast_origin: state.forecastOrigin,
      information_cutoff: state.informationCutoff, scenario: editor.scenario,
  ...
  async function resume() {
    ...
    try {
      ...
      if (!disposed && state.activeRun?.id === id) { state.activeRun = run; void poll(id) }
    } catch (cause) {
  ...
  function beforeUnload(event: BeforeUnloadEvent) {
    if (dirtySinceResult.value) { event.preventDefault(); event.returnValue = '' }
  }
  onMounted(() => window.addEventListener('beforeunload', beforeUnload))
  onBeforeUnmount(() => {
    disposed = true; loadGeneration++; stopPolling()
    window.removeEventListener('beforeunload', beforeUnload)
  })
  return {
    editor, revisions, currentRevision, currentRun, resultRun, result, selectedIds,
    loading, submitting, error, pollError, busy, validation, dirtySinceResult,
    generate, resume, loadScope, markDirty,
